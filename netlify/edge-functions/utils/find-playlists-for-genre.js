...
export async function findPlaylistsForGenre({ genre, spotifyAccessToken }) {
  const genreInList = genresJson.find((g) => g.name === genre);

  if (!genreInList)
    throw new Error(`Could not find ${genre} in list of ENAO genres.`);
